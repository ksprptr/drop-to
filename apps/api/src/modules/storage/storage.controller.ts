 ...
 *
 * Depends only on the {@link StorageProvider} abstraction (via `STORAGE_PROVIDER`),
 * never on a concrete backend. Route paths stay `/drive/*` for API compatibility.
 */
@ApiTags('Drive')
...
export class StorageController {
  ...
  /**
   * Controller to list the authorized root folders
   */
  @ApiOperation({ summary: 'List authorized root folders' })
  @ApiOkResponse({ type: [AllowedFolderEntity], description: 'Successful' })
  @Get('folders')
  async getFolders(): Promise<AllowedFolderEntity[]> {
    return this.storage.listRoots();
  }

  /**
   * Controller to list the contents of an authorized folder
   */
  ...
  async getContents(@Param('id') id: string): Promise<DriveEntryEntity[]> {
    return this.storage.listContents(id);
  }
  ...
  async downloadFile(@Param('id') id: string, @Res() res: Response): Promise<void> {
    const { stream, name, mimeType, size } = await this.storage.downloadFile(id);
  ...
  async downloadFolder(@Param('id') id: string, @Res() res: Response): Promise<void> {
    const { archive, name } = await this.storage.createFolderArchive(id);
  ...
  /**
   * Controller to create a subfolder inside an authorized folder
   */
  ...
  @ApiForbiddenResponse({ type: ResponseEntity, description: 'Folder outside authorized tree' })
  @Post('folders/:id/subfolder')
  async createSubfolder(
    @Param('id') id: string,
    @Body() createSubfolderDto: CreateSubfolderDto,
  ): Promise<DriveEntryEntity> {
    return this.storage.createFolder(id, createSubfolderDto.name);
  }

  /**
   * Controller to upload a file into an authorized folder
   */
  ...
  async uploadFile(@Param('id') id: string, @Req() req: Request): Promise<UploadResultEntity> {
    // If the client disconnects *mid-upload* (e.g. a page refresh) abort the Drive
  ...
  @ApiConflictResponse({ type: ResponseEntity, description: 'Authorized root folders cannot be deleted' })
  @HttpCode(204)
  @Delete('files/:id')
